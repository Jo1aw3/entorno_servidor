...
class Jokalari_Modeloa {
    ...
    public function datosPreguntas() {
        try {
            $sql = "SELECT prematuro, puntuazio_max FROM jokalariak ORDER BY puntuazio_max DESC;";
            $resultado = $this->mysqli->query($sql);
            $lista = array();
            foreach($resultado as $linea){
                $lista[$linea['erabiltzailea']]=$linea['puntuazio_max'];
            }
            return $lista;
