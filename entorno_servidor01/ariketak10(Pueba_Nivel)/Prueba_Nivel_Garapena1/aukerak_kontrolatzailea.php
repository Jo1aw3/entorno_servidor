...
if (isset($_POST['botoia']) && !$_SESSION["balioztatua"]) {
    if($jok_model->balioztatzea($_POST['erab'], $_POST['ph'])){
        ...
        ?>
        <h3 style="color: red;">Intentalo de nuevo, datos de usuarios incorrectos. </h3>
        <?php
...
if ($_SESSION["balioztatua"] && isset($_POST['jokatu_botoia'])){
    $puntuak=0;
    $kont=0;
    foreach ($emaitzen_arraya as $pregunta => $respuesta){
        if ($_POST['galdera'.$kont++]==$respuesta){
            echo ($pregunta ." galderaren erantzuna ". $respuesta." da. Beraz zuzena da, oso ondo puntuak lortu dituzu. <br><br>");
            $puntuak=$puntuak + 3;
